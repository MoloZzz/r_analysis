...
# -------------------------------
# 4. Проста стовпчикова діаграма
# -------------------------------
ggplot(agg, aes(x = Group, y = mean_value)) +
  geom_col(fill = "steelblue") +
  labs(
    title = "Проста стовпчикова діаграма",
    x = "Група", y = "Середнє значення"
  ) +
  theme_minimal()

# -------------------------------
# 5. Згрупована діаграма
# -------------------------------
ggplot(agg, aes(x = Group, y = mean_value, fill = Category)) +
  geom_col(position = position_dodge()) +
  labs(
    title = "Згрупована діаграма",
    x = "Група", y = "Середнє значення"
  ) +
  theme_minimal()
...
# -------------------------------
# 7. Нормована (100%) діаграма
# -------------------------------
agg_percent <- agg %>%
  group_by(Group) %>%
  mutate(pct = mean_value / sum(mean_value))

ggplot(agg_percent, aes(x = Group, y = pct, fill = Category)) +
  ...
  labs(title = "Нормована (100%) стовпчикова діаграма") +
  theme_minimal()

# -------------------------------
# 8. Стовпчики з похибками (SE)
# -------------------------------
ggplot(agg, aes(x = Group, y = mean_value, fill = Category)) +
  ...
  labs(title = "Стовпчики з похибками (SE)") +
  theme_minimal()
